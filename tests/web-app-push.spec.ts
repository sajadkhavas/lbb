...
test("Web App settings route exposes install and notification controls", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { level: 1, name: "Web App و اعلان‌های سفارش" }),
  ).toBeVisible();
  ...
  await expect(
    page.getByText("مجوز اعلان فقط وقتی خودت دکمه فعال‌سازی را بزنی درخواست می‌شود."),
  ).toBeVisible();
...
test("manifest remains installable and links to Web App settings", async ({ request }) => {
  ...
  expect(
    manifest.shortcuts.some((shortcut: { url?: string }) => shortcut.url === "/web-app"),
  ).toBeTruthy();
});

test("service worker has visible Push and safe notification-click handling", async ({
  request,
}) => {
