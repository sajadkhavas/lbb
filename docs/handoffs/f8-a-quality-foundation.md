...
- `src/routes/index.tsx` does not currently emit the homepage canonical URL. Add an absolute canonical generated by `canonical("/")` in Final Review; the F8-A SSR test records this as an explicit handoff attachment rather than changing the feature-owned route.
...
## ESLint integration diagnostics

`npm run lint` runs ESLint across the complete repository, stores the JSON report in `artifacts/lint`, and fails on any F8-A-owned error. Existing formatting and feature-owned lint findings are preserved for integration instead of changing parallel feature work. `npm run lint:integration` must pass without ownership exceptions before release.
...
- Add the absolute homepage canonical and replace unverified homepage schema after the feature branch integration.
- Run `npm run typecheck:integration` and `npm run lint:integration` after all feature branches merge and resolve every reported diagnostic.
