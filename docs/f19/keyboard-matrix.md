...
| Lookbook lightbox             | Enter open, Tab/Shift+Tab, ArrowLeft/Right, Escape             | focus enters; wraps; restores opener on close                   | `f19-interactions`                                          | Gated                                 |
...
The Lookbook lightbox is included in the same focus-wrap regression as the shared overlays. Its current Chromium runtime behavior satisfies entry, Tab/Shift+Tab containment, arrows, Escape and opener restoration, so F19-A does not carry a separate Lookbook focus debt into F19-B.
