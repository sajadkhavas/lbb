...
for (const width of widths) {
  for (const route of routes) {
    ...
    const imagesOkay = await page.locator("main img").evaluateAll((images) =>
      images.every((image) => image.complete && image.naturalWidth > 0 && Boolean(image.getAttribute("alt"))),
    );
    if (!imagesOkay) failures.push(`${width}px ${route}: image failed or missing alt`);
