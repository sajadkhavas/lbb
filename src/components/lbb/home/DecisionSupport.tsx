        ...
        <Shell>
          <header className="grid gap-5 border-b border-hairline pb-7 md:grid-cols-[auto_minmax(0,1fr)] md:items-end md:gap-10">
            <div className="flex items-center gap-3">
              ...
              <TechLabel>{decisionSupport.label}</TechLabel>
            </div>
            <div className="md:justify-self-end md:text-left">
              ...
              <p className="mt-3 max-w-[58ch] text-sm leading-7 text-metal">{decisionSupport.lede}</p>
            </div>
          </header>
          ...
          <div className="mt-8 grid gap-4 lg:grid-cols-[minmax(300px,0.72fr)_minmax(0,1.28fr)]">
            <div className="rounded-2xl border border-hairline bg-obsidian p-5 text-bone shadow-raised md:p-6">
              ...
              <ul className="mt-5 space-y-4">
                {decisionSupport.checks.map((item) => (
            ...
            <nav aria-label="راهنماهای تصمیم خرید" className="grid gap-3 md:grid-cols-3">
              {decisionSupport.links.map((item, index) => (
                <a key={`${item.href}-${item.label}`} href={item.href} className={CARD_CLASS}>
    ...
    >
      <Shell>
        <header className="grid gap-5 border-b border-hairline pb-7 md:grid-cols-[auto_minmax(0,1fr)] md:items-end md:gap-10">
          ...
          <div className="md:justify-self-end md:text-left">
            <h2 id="decision-support-title" className="text-display-3 text-bone">قبل از انتخاب، جواب‌ها را داشته باش</h2>
            <p className="mt-3 max-w-[58ch] text-sm leading-7 text-metal">اگر بین دو سایز یا مدل مرددی، این راهنماها انتخاب را سریع‌تر و مطمئن‌تر می‌کنند.</p>
          </div>
        </header>
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-[minmax(300px,0.72fr)_minmax(0,1.28fr)]">
          ...
          <nav aria-label="راهنماهای تصمیم خرید" className="grid gap-3 md:grid-cols-3">
            {HOME_DECISION_LINKS.map((item, index) =>
              item.kind === "journal" ? (
                <Link key={item.label} to="/journal/$slug" params={item.params} className={CARD_CLASS}>
                  <DecisionCard index={index} {...item} />
                </Link>
              ) : (
