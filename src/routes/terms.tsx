...
const DESC = "شرایط استفاده از وب‌سایت LBB و وضعیت انتشار شرایط تجاری؛ سیاست‌های منتشرنشده به‌عنوان تعهد فروش نمایش داده نمی‌شوند.";
...
export const Route = createFileRoute("/terms")({
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: pageMeta({ title, description, path: "/terms", robots: page ? undefined : ROBOTS.NOINDEX_FOLLOW }),
  ...
  return (
    <>
      ...
      <main dir="rtl" className="min-h-screen overflow-x-clip bg-obsidian pb-28 pt-16" data-storefront-source={source}>
        <div className="hairline-b"><Shell className="py-3"><Breadcrumb items={[{ label: "خانه", href: "/" }, { label: page?.title || "شرایط استفاده" }]} /></Shell></div>
        ...
        <header className="mx-auto max-w-[860px] px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3">
            ...
            <StatusTag tone={publishedFromAdmin ? "success" : "warning"}>{publishedFromAdmin ? "منتشرشده از پنل" : "شرایط نهایی منتشر نشده"}</StatusTag>
          </div>
          <p className="mt-4 max-w-[68ch] text-sm leading-8 text-metal">{page?.excerpt || "تا انتشار نسخه نهایی از پنل، Frontend شرط تجاری، مهلت، هزینه یا تعهد جدیدی ایجاد نمی‌کند."}</p>
        </header>
        ...
        <div className="mx-auto max-w-[860px] px-4 pb-16 md:px-8">
          ...
          <section className="mt-8 grid gap-4 md:grid-cols-2" aria-label="وضعیت عملیاتی Backend">
            <div className="rounded-2xl border border-hairline bg-carbon p-5">
              ...
              <p className="mt-3 text-sm leading-7 text-metal">{runtime.checkoutEnabled ? "Backend ثبت سفارش را فعال اعلام کرده است." : "Backend ثبت سفارش را غیرفعال اعلام کرده است."}</p>
            </div>
            <div className="rounded-2xl border border-hairline bg-carbon p-5">
              <p className="tech text-signal">PAYMENT / RUNTIME</p>
              <h2 className="mt-2 text-lg font-bold text-bone">پرداخت</h2>
              <p className="mt-3 text-sm leading-7 text-metal">{runtime.payment.enabled ? `Backend درگاه «${runtime.payment.provider}» را فعال اعلام کرده است؛ موفقیت پرداخت فقط پس از Verify سمت سرور معتبر است.` : "هیچ درگاه پرداخت فعال از Backend اعلام نشده است."}</p>
            </div>
            <div className="rounded-2xl border border-hairline bg-carbon p-5 md:col-span-2">
              ...
              <p className="mt-3 text-sm leading-7 text-metal">{returnsPublished ? "سیاست مرجوعی در پنل فعال و تأیید شده است. جزئیات کامل در صفحه ارسال و مرجوعی نمایش داده می‌شود." : "سیاست مرجوعی هنوز در پنل به وضعیت فعال و تأییدشده نرسیده است."}</p>
              <Link to="/shipping-returns" className={`${CtaClasses("line")} mt-4`}>مشاهده ارسال و مرجوعی</Link>
            </div>
          </section>

          <div className="mt-8 flex flex-wrap gap-3">
            <Link to="/privacy" className={CtaClasses("line")}>حریم خصوصی</Link>
            <Link to="/contact" className={CtaClasses("signal")}>تماس و پشتیبانی</Link>
          </div>
        </div>
      </main>
      <Footer />
