...
import { Band, CtaClasses, SectionHead, Shell, StatusTag, TechLabel } from "@/components/lbb/ui/primitives";
...
function ProductPage() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([entry]) => setStickyVisible(!entry.isIntersecting), { rootMargin: "0px" });
  ...
  return (
    <>
      ...
      <main dir="rtl" className="min-h-screen bg-obsidian pb-bottombar pt-16 md:pb-0">
        ...
        <Shell as="section" className="grid grid-cols-1 gap-8 pb-12 md:grid-cols-[minmax(0,62%)_minmax(0,38%)] md:gap-10">
          <Gallery slug={p.slug} name={p.name} />

          <div className="flex min-w-0 flex-col gap-5 md:sticky md:top-20 md:self-start">
            <div>
              <TechLabel tone="signal">{cat.nameFa} / {p.latinName}</TechLabel>
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="flex flex-wrap items-baseline gap-3">
                ...
                {p.originalPrice ? <span className="num text-sm text-mute line-through">{fmtToman(p.originalPrice)}</span> : null}
              </div>
              <button
                type="button"
                onClick={() => {
              ...
              <div className="flex flex-wrap gap-2">
                {p.colors.map((item) => (
                  <button
                    ...
                    style={{ background: item, borderColor: color === item ? "var(--lbb-signal)" : "var(--lbb-hairline)" }}
                  />
                ))}
              </div>
            </fieldset>
            ...
            <fieldset>
              <div className="mb-2 flex items-center justify-between gap-3">
                ...
                <SizeGuideDialog trigger={<button type="button" className="min-h-11 tech text-signal focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-signal">راهنمای سایز</button>} />
              </div>
              <div ref={sizeGroupRef} className="flex flex-wrap gap-2" aria-describedby={sizeError ? "pdp-size-error" : undefined}>
                {p.sizes.map((item) => {
              ...
              {sizeError ? <p id="pdp-size-error" role="alert" className="mt-2 text-xs font-semibold text-signal">لطفاً پیش از افزودن به سبد، یک سایز موجود را انتخاب کنید.</p> : null}
              {!p.inStock ? <p className="mt-2 text-xs leading-6 text-metal">این محصول فعلاً ناموجود است؛ برای مشاهدهٔ گزینه‌های مشابه به بخش پیشنهادها بروید.</p> : null}
            </fieldset>

            <div className="flex items-center gap-3">
              <span className="text-xs font-semibold text-bone">تعداد:</span>
              <div className="flex items-center border border-hairline">
                <button type="button" onClick={() => setQty((value) => Math.max(1, value - 1))} disabled={qty <= 1} aria-label="کاهش تعداد" className="tap-target text-lg text-bone disabled:opacity-35">−</button>
                <output className="num w-10 text-center text-sm text-bone" aria-live="polite">{qty}</output>
                <button type="button" onClick={() => setQty((value) => Math.min(MAX_QTY, value + 1))} disabled={qty >= MAX_QTY} aria-label="افزایش تعداد" className="tap-target text-lg text-bone disabled:opacity-35">+</button>
              </div>
            </div>

            <div ref={addBtnRef}>
              <button type="button" onClick={onAdd} disabled={!p.inStock} className={`${CtaClasses("signal")} h-14 w-full`}>
                {added ? <><Check size={18} aria-hidden="true" /><span>به سبد اضافه شد</span></> : <><ShoppingBag size={18} aria-hidden="true" /><span>{p.inStock ? "افزودن به سبد خرید" : "ناموجود"}</span></>}
              </button>
            </div>
          </div>
        </Shell>
          ...
          <Shell className="max-w-[760px]">
            <Accordion type="single" collapsible defaultValue="details" className="text-bone">
              <AccordionItem value="details" className="border-hairline">
                <AccordionTrigger className="text-sm font-bold text-bone hover:no-underline">جزئیات</AccordionTrigger>
              ...
              <AccordionItem value="care" className="border-hairline">
                <AccordionTrigger className="text-sm font-bold text-bone hover:no-underline">جنس و مراقبت</AccordionTrigger>
                <AccordionContent>
                  <div className="flex flex-col gap-4 text-sm leading-8 text-metal">
                    <table className="w-full max-w-[420px] text-sm">
                      <tbody>
                        <tr className="border-b border-hairline"><th scope="row" className="py-2 text-start font-normal text-mute">جنس</th><td className="py-2">{p.material}</td></tr>
                        <tr className="border-b border-hairline"><th scope="row" className="py-2 text-start font-normal text-mute">سایزبندی</th><td className="num py-2">{p.sizes.join(" · ")}</td></tr>
                        <tr><th scope="row" className="py-2 text-start font-normal text-mute">کد محصول</th><td className="num py-2">{p.sku}</td></tr>
                      </tbody>
                    </table>
                    <ul className="flex flex-col gap-1.5">
                      {p.care.map((care) => <li key={care} className="flex gap-2"><span aria-hidden="true" className="mt-3 h-px w-3 shrink-0 bg-signal" />{care}</li>)}
                    </ul>
                  </div>
                </AccordionContent>
              </AccordionItem>
              <AccordionItem value="fit" className="border-hairline">
                <AccordionTrigger className="text-sm font-bold text-bone hover:no-underline">راهنمای فیت</AccordionTrigger>
                <AccordionContent><FitGuide product={p} /></AccordionContent>
              </AccordionItem>
              <AccordionItem value="shipping" className="border-hairline">
                <AccordionTrigger className="text-sm font-bold text-bone hover:no-underline">ارسال و بازگشت</AccordionTrigger>
                <AccordionContent>
          ...
          <Band label="YOU MAY ALSO LIKE">
            <Shell>
              ...
              <div className="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-5">
                {related.map((product: Product) => <ProductCard key={product.id} p={product} />)}
              </div>
            </Shell>
          </Band>
        ) : null}
      ...
      <StickyBuyBar visible={stickyVisible} name={p.name} price={p.price} inStock={p.inStock} selectedSize={size} onAdd={onAdd} />
      <Footer theme="light" />
      <MobileBottomBar />
    </>
  );
