...
requireValue(
  ...
  "manifest start_url/scope must be root",
);
...
requireValue(
  ...
  "service worker must handle notification clicks",
);
requireValue(
  worker.includes("safeNotificationUrl"),
  "notification navigation must be same-origin sanitized",
);
...
requireValue(
  root.includes("prepareWebAppInstall()"),
  "root must prepare install handling before route interaction",
);
requireValue(pushApi.includes('"/api/web-push/'), "Push API must use additive non-v1 namespace");
requireValue(
  pushApi.includes('credentials: "include"'),
  "Push API must preserve authenticated cookies",
);
requireValue(
  pushApi.includes("ensureBackendCsrf"),
  "Push mutations must use Sanctum CSRF bootstrap",
);
requireValue(
  panel.includes("Notification.requestPermission()"),
  "permission request must exist in explicit Push control",
);
requireValue(
  panel.includes("userVisibleOnly: true"),
  "PushManager subscription must require visible notifications",
);
requireValue(
  panel.includes("applicationServerKey"),
  "PushManager subscription must use VAPID public key",
);
requireValue(
  panel.includes("onClick={() => void enablePush()}"),
  "permission flow must be tied to explicit user action",
);
requireValue(
  !root.includes("Notification.requestPermission"),
  "root must never auto-prompt notification permission",
);
requireValue(
  navigation.includes('to: "/web-app"'),
  "Web App settings must be discoverable in personal navigation",
);
