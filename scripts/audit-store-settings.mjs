...
for (const required of [
  ...
  if (!readiness.includes(required)) failures.push(`Launch-readiness contract is missing: ${required}`);
}
...
for (const required of [
  ...
  if (!storefrontControl.includes(required)) failures.push(`Final storefront control missing: ${required}`);
}
...
for (const required of ["useStorefrontControl", "policies.enamad", 'rel="noopener noreferrer"']) {
  if (!trustMarks.includes(required)) failures.push(`Backend-driven trust mark missing: ${required}`);
}
...
for (const required of ["getDeliveryOptions", "useStorefrontControl", "policies.returns", "shippingCards"]) {
  if (!shippingReturns.includes(required)) failures.push(`Backend-authoritative shipping/returns missing: ${required}`);
}
if (/2_000_000|60_000|۷ روز ضمانت بازگشت/.test(shippingReturns)) {
  failures.push("Unsupported fixed shipping/returns promise remains in final public policy surface.");
}
...
for (const required of [
  ...
  if (!contact.includes(required)) failures.push(`Admin-driven contact control missing: ${required}`);
}

for (const required of ["resolveOptionalStorefrontPage(\"size-guide\")", 'control.source === "live"', "NOINDEX_FOLLOW"]) {
  if (!sizeGuide.includes(required)) failures.push(`Admin-driven size guide boundary missing: ${required}`);
}

for (const required of ["runtime.checkoutEnabled", "runtime.payment.enabled", "policies.returns"]) {
  if (!terms.includes(required)) failures.push(`Backend-authoritative Terms status missing: ${required}`);
}
for (const required of ["runtime.checkoutEnabled", "runtime.payment.enabled", "useStorefrontControl"]) {
  if (!privacy.includes(required)) failures.push(`Backend-authoritative Privacy status missing: ${required}`);
}
...
for (const required of ["getCollection", "resolveLiveFeaturedStory", "featuredStory", "listStorefrontCategories", "listProducts"]) {
  if (!home.includes(required)) failures.push(`Home live loader missing backend authority: ${required}`);
}

for (const required of ["safePublicHref", 'url.protocol === "https:"', 'href.startsWith("/")', '!href.startsWith("//")']) {
  if (!publicHref.includes(required)) failures.push(`Safe admin-link boundary missing: ${required}`);
}
...
for (const method of canonicalDeliveryMethods) {
  if (!backendApi.includes(`\"${method}\"`)) failures.push(`Delivery contract missing method: ${method}`);
}
...
for (const required of ["policyEligible", '"freight_collect"', "isFree", "feeNotice"]) {
  if (!backendDelivery.includes(required)) failures.push(`Delivery response contract missing: ${required}`);
}

for (const forbidden of ["FREE_SHIPPING_THRESHOLD", "STANDARD_SHIPPING_FEE", "saveDemoOrder", "createDemoOrderRef", "findDemoOrder"]) {
  if (commerce.includes(forbidden)) failures.push(`Invented/demo commerce behavior remains: ${forbidden}`);
}
if (!commerce.includes("orderSubmissionReady: false")) failures.push("Public settings alone must not claim order-submission readiness.");
if (!commerce.includes("paymentVerificationReady: false")) failures.push("Public settings alone must not claim payment-verification readiness.");

for (const forbidden of ["saveDemoOrder", "createDemoOrderRef", "shippingFeeFor", "findDemoOrder"]) {
  if (checkout.includes(forbidden)) failures.push(`Checkout still contains invented/demo behavior: ${forbidden}`);
}
for (const required of [
...
if (liveStart < 0 || prototypeStart < 0 || prototypeStart <= liveStart) {
  ...
  if (!/<form\b/i.test(liveCheckout)) failures.push("Live checkout must collect data only for authoritative Backend quote.");
  for (const required of ["createCheckoutQuote", "commitCheckout", "initiatePayment"]) {
    if (!liveCheckout.includes(required)) failures.push(`Live checkout does not use Backend operation: ${required}`);
  }
}
...
console.log("Final admin-driven storefront, safe public-link and Backend commerce authority audit passed.");
