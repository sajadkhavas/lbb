...
const binary = path.join(root, "node_modules", ".bin", process.platform === "win32" ? "knip.cmd" : "knip");
...
const markdown = [
  ...
  result.stderr?.trim() ? `- Diagnostics: \`${result.stderr.trim().replaceAll("`", "'")}\`` : "- Diagnostics: none",
  "",
...
if (result.error) {
  ...
  console.log(`Dead-code report generated (Knip exit ${result.status}). Findings do not fail F8-A.`);
}
